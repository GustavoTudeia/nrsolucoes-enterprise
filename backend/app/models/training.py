...
class ActionItemEnrollment(
    ...
    action_item_id = Column(
        GUID(),
        ForeignKey("action_item.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    content_assignment_id = Column(
        GUID(),
        ForeignKey("content_assignment.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    certificate_id = Column(
        GUID(),
        ForeignKey("training_certificate.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    content_assignment = relationship(
        "ContentAssignment", foreign_keys=[content_assignment_id]
    )
    certificate = relationship("TrainingCertificate", foreign_keys=[certificate_id])
...
class TrainingCertificate(Base, UUIDPrimaryKeyMixin, TimestampMixin, TenantScopedMixin):
    ...
    enrollment_id = Column(
        GUID(),
        ForeignKey("action_item_enrollment.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    action_item_id = Column(
        GUID(),
        ForeignKey("action_item.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    employee_id = Column(
    ...
    content_id = Column(
        GUID(), ForeignKey("content_item.id", ondelete="SET NULL"), nullable=True
    )
