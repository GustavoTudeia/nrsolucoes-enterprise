    ...
    <div className="grid gap-6 lg:grid-cols-[420px,1fr]">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Button disabled={saving} onClick={async()=>{ try{ setSaving(true); await createErgonomicAssessment(form); toast.success("Avaliação criada"); setForm({ assessment_type: form.assessment_type || "AEP", psychosocial_factors: [], findings: [], recommendations: [] }); await refresh(); } catch(e:any){ toast.error(e?.message || "Falha ao criar avaliação"); } finally { setSaving(false); } }}>Salvar avaliação</Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
