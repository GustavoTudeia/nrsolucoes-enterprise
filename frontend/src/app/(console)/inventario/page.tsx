    ...
    <div className="grid gap-6 lg:grid-cols-[420px,1fr]">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Button disabled={saving} onClick={async () => { try { setSaving(true); await createInventoryItem(form); toast.success("Item criado"); setForm({ hazard_group: form.hazard_group, severity: 3, probability: 3, existing_controls: [], proposed_controls: [], evidence_requirements: [] }); await refresh(); } catch (e: any) { toast.error(e?.message || "Falha ao criar item"); } finally { setSaving(false); } }}>Salvar item</Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
    ...
    <Card>
      ...
      <CardContent className="grid gap-6 lg:grid-cols-[360px,1fr]">
        <div className="space-y-3">
          ...
          <Button onClick={async()=>{ try { await createPgrApproval(approvalForm); toast.success("Versão formal criada"); setApprovalForm({ document_scope: approvalForm.document_scope || "inventory" }); await refresh(); } catch(e:any){ toast.error(e?.message || "Falha ao formalizar versão"); } }}>Formalizar versão</Button>
        </div>
