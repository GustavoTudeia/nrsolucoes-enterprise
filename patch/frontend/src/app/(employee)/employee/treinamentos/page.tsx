...
export default function TreinamentosPage() {
  ...
  async function loadTrainings() {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/employee/me/trainings`, {
  ...
  async function handleStart(enrollmentId: string) {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/employee/me/trainings/${enrollmentId}/start`,
        {
          method: "POST",
          headers: { "Authorization": `Bearer ${token}` },
        }
  ...
  async function handleComplete(enrollmentId: string) {
    try {
      const token = localStorage.getItem("employee_token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/employee/me/trainings/${enrollmentId}/complete`,
        {
          method: "POST",
          headers: { "Authorization": `Bearer ${token}` },
        }
