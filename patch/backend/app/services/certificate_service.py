...
from app.services.storage import create_upload_url, upload_file_directly
...
class CertificateService:
    ...
    def create_certificate(
        ...
        action_item = self.db.query(ActionItem).filter(ActionItem.id == enrollment.action_item_id).first()
        
        content = None
        if action_item.education_ref_id:
            content = self.db.query(ContentItem).filter(ContentItem.id == action_item.education_ref_id).first()
        
        action_plan = None
        if action_item.action_plan_id:
            action_plan = self.db.query(ActionPlan).filter(ActionPlan.id == action_item.action_plan_id).first()
        
        # Buscar dados da empresa
        tenant = self.db.query(Tenant).filter(Tenant.id == enrollment.tenant_id).first()
        cnpj = None
        if tenant:
            cnpj = self.db.query(CNPJ).filter(CNPJ.tenant_id == tenant.id, CNPJ.is_active == True).first()
    ...
    def generate_pdf(self, certificate: TrainingCertificate) -> Tuple[bytes, str]:
        ...
        elements.append(Paragraph("Programa de Gerenciamento de Riscos - NR-1", subtitle_style))
        ...
        if certificate.training_description:
            elements.append(Paragraph(
                ...
                ParagraphStyle('Desc', parent=body_style, fontSize=12, textColor=colors.HexColor('#4a5568'))
            ))
        
        elements.append(Spacer(1, 15))
        
        # Informações
        info_text = f"Concluído em: {certificate.training_completed_at.strftime('%d/%m/%Y')}"
        ...
        if certificate.risk_dimension:
            ...
            dim_name = dimension_names.get(certificate.risk_dimension, certificate.risk_dimension)
        ...
        if certificate.issuer_name:
            elements.append(Paragraph(
                f"Emitido por: {certificate.issuer_name}",
                footer_style
            ))
        ...
        if certificate.validation_code:
            ...
            elements.append(Paragraph(
                ...
                ParagraphStyle('Code', parent=footer_style, fontName='Courier', fontSize=11)
            ))
    ...
    def _generate_simple_pdf(self, certificate: TrainingCertificate) -> Tuple[bytes, str]:
...
"""
    ...
    def save_pdf(self, certificate: TrainingCertificate) -> str:
        ...
        # Upload direto
        try:
            upload_file_directly(storage_key, pdf_content, "application/pdf")
        except Exception:
    ...
    def validate_certificate(self, validation_code: str) -> Optional[TrainingCertificate]:
    ...
    def validate_by_number(self, certificate_number: str) -> Optional[TrainingCertificate]:
