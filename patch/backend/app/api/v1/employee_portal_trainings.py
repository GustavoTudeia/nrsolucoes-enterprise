...
def get_employee_from_token(token: str, db: Session) -> Employee:
    ...
    employee = db.query(Employee).filter(Employee.id == portal_token.employee_id).first()
    if not employee:
...
def start_my_training(
    ...
    if enrollment.status not in ("pending", "in_progress"):
        raise BadRequest(f"Não é possível iniciar treinamento com status {enrollment.status}")
...
def update_my_training_progress(
    ...
    if enrollment.status in ("completed", "cancelled"):
        raise BadRequest(f"Não é possível atualizar treinamento com status {enrollment.status}")
    
    # Calcular progresso percentual
    progress_percent = 0
    if payload.duration_seconds and payload.duration_seconds > 0:
        progress_percent = min(100, int(payload.position_seconds / payload.duration_seconds * 100))
...
def get_training_content_access(
    ...
    content = db.query(ContentItem).filter(ContentItem.id == action_item.education_ref_id).first()
    if not content:
