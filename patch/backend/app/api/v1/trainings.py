...
def list_enrollments(
    ...
    enrollments = (
        query
        .order_by(ActionItemEnrollment.due_date.asc().nullslast())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def update_enrollment(
    ...
    enrollment = (
        db.query(ActionItemEnrollment)
        .filter(ActionItemEnrollment.id == enrollment_id, ActionItemEnrollment.tenant_id == tenant_id)
        .first()
    )
    if not enrollment:
...
def complete_enrollment(
    enrollment_id: UUID,
    generate_certificate: bool = Query(True, description="Gerar certificado automaticamente"),
    valid_months: Optional[int] = Query(None, ge=1, le=120, description="Validade do certificado em meses"),
    db: Session = Depends(get_db),
    _sub_ok: None = Depends(require_active_subscription),
    user=Depends(require_any_role([ROLE_OWNER, ROLE_TENANT_ADMIN, ROLE_CNPJ_MANAGER, ROLE_UNIT_MANAGER])),
    tenant_id: UUID = Depends(tenant_id_from_user),
...
def list_certificates(
    ...
    query = db.query(TrainingCertificate).filter(TrainingCertificate.tenant_id == tenant_id)
...
def get_certificate(
    ...
    cert = (
        ...
        .filter(TrainingCertificate.id == certificate_id, TrainingCertificate.tenant_id == tenant_id)
        .first()
    )
    if not cert:
...
def download_certificate_pdf(
    ...
    if cert.pdf_storage_key:
        try:
            ...
            return {"download_url": presigned.url, "expires_in_seconds": presigned.expires_in}
        except Exception:
            pass
...
def get_training_report(
    ...
    certificates = {
        str(cert.id): cert
        for cert in db.query(TrainingCertificate).filter(TrainingCertificate.id.in_(certificate_ids)).all()
    }
    ...
    for enrollment in enrollments:
        emp = employees.get(str(enrollment.employee_id))
        cert = certificates.get(str(enrollment.certificate_id)) if enrollment.certificate_id else None
